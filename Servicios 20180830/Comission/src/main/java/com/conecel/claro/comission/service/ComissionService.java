			...
			for (PreComission preComision : preComissions) {
				...
				for (ComissionRules regla : reglas) {
					...
					if ((preComision.getDeliveryDays() >= regla.getStartDay())
						...
						float subComision = Float.parseFloat(comision);

						System.out.println("Valor comisision: " + subComision);
	...
	public GeneratedComissionResponse generaComision2(ComissionPeriod comissionPeriod) {
		...
		try {
			...
			for (PreComission2 preComision : preComissions) {
				...
				/*
				 * String comision = microEis.obtieneComision(comissionInfoService.getUrlMEis(),
				 * preComision.getIdOrder(), preComision.getAccount(),
				 * comissionInfoService.getInformationService(),
				 * comissionInfoService.getDataSource());
				 * 
				 * System.out.println("Resultado jeis: " + comision);
				 */
				...
				for (ComissionRules regla : reglas) {
					...
					if ((preComision.getDeliveryDays() >= regla.getStartDay())
						...
						String comision = microEis.obtieneComision(comissionInfoService.getUrlMEis(),
								preComision.getIdOrder(), preComision.getAccount(),
								comissionInfoService.getInformationService(), comissionInfoService.getDataSource());

						System.out.println("Resultado jeis: " + comision);
						
						System.out.println("No debe ingresar: " + preComision.getIdOrder());

						if (!"".equals(comision)) {
							
							System.out.println("Ingresa: " + preComision.getIdOrder());
							...
							comission.setDeliveryDays(preComision.getDeliveryDays());
							comission.setSubComission(subComision);
							comission.setPenalty(penalizacion);
							comission.setPenaltyValue(multa);
