...
public class SalesResultDAO implements ISalesResultDAO {
	...
	public List<SalesResult> obtieneDetalleVenta(PreSalesInfo preSalesInfo) {
		...
		try {
			...
			if (!"".equals(preSalesInfo.getLotId())) {

				queryGenerated = queryGenerated + " and lot.numero_lote= upper(:lote)";
				parametros.put("lote", preSalesInfo.getLotId());

			}

			if (!"".equals(preSalesInfo.getStatus()) && !"PENDIENTE".equals(preSalesInfo.getStatus())) {

				queryGenerated = queryGenerated + " and lot.estado= upper(:estado)";
				parametros.put("estado", preSalesInfo.getStatus());

			}

			if (!"".equals(preSalesInfo.getStatus()) && "PENDIENTE".equals(preSalesInfo.getStatus())) {

				queryGenerated = queryGenerated + "  AND VTA.ESTADO='PENDIENTE'\r\n" + 
						"   AND NVL(LOT.ESTADO,'PENDIENTE') NOT IN ('REGULARIZADO','RECHAZADO')";

			}

			queryGenerated = queryGenerated + "  order by Estado asc, vta.numero_orden asc";

			System.out.println("query: " + queryGenerated);
			
			System.out.println("Estado: " + preSalesInfo.getStatus());

			TypedQuery<SalesResult> query = (TypedQuery<SalesResult>) this.getEntityManager()
					.createNativeQuery(queryGenerated, SalesResult.class);

			parametros.forEach((k, v) -> query.setParameter(k, v));

			resultado = query.getResultList();

			log.info("Venta consulta exitosamente!");

		} catch (Exception e) {
			e.printStackTrace();
			log.error("Error en acceder a datos de ventas: ", e);
			return null;
		}
